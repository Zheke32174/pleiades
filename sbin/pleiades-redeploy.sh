...
WORK_DIR="/tmp/pleiades-redeploy-$$"
BACKUP_DIR="/tmp/pleiades-backup-$$"
SELF="$0"
...
mkdir -p "$BACKUP_DIR"
if [[ -d "$MAIA_DIR" ]]; then
    cp -a "$MAIA_DIR/keys" "$BACKUP_DIR/" 2>/dev/null || true
    cp -a "$MAIA_DIR/escrow" "$BACKUP_DIR/" 2>/dev/null || true
    cp -a "$MAIA_DIR/github_drop_url" "$BACKUP_DIR/" 2>/dev/null || true
...
rm -f /usr/local/bin/pleiades-* 2>/dev/null || true
rm -f /usr/local/sbin/install-*-omniversal.sh 2>/dev/null || true
rm -f /etc/systemd/system/pleiades-*.service 2>/dev/null || true
rm -f /etc/systemd/system/maia-*.service 2>/dev/null || true
systemctl daemon-reload 2>/dev/null || true

# Step 4: Restore critical state into fresh clone
if [[ -d "$BACKUP_DIR/keys" ]]; then
    mkdir -p "$WORK_DIR/var/lib/.maia/keys"
    cp -a "$BACKUP_DIR/keys/." "$WORK_DIR/var/lib/.maia/keys/" 2>/dev/null || true
fi
if [[ -d "$BACKUP_DIR/escrow" ]]; then
    mkdir -p "$WORK_DIR/var/lib/.maia/escrow"
    cp -a "$BACKUP_DIR/escrow/." "$WORK_DIR/var/lib/.maia/escrow/" 2>/dev/null || true
fi
if [[ -f "$BACKUP_DIR/github_drop_url" ]]; then
    mkdir -p "$WORK_DIR/var/lib/.maia"
    cp "$BACKUP_DIR/github_drop_url" "$WORK_DIR/var/lib/.maia/" 2>/dev/null || true
fi
...
systemctl daemon-reload 2>/dev/null || true
systemctl restart pleiades-forensic-scanner.service 2>/dev/null || true
systemctl restart pleiades-chaos-monkey.service 2>/dev/null || true
