...
# ALCYONE_ID
# ==================================================================
# ALCYONE – OMNIVERSAL (WSL / DGX Spark / VPS)
# ==================================================================
...
func reportToPleiades Nexus(msg string) {
    f, err := os.OpenFile("/run/pleiades/pleiades_nexus_fifo", os.O_WRONLY|os.O_APPEND|syscall.O_NONBLOCK|os.O_CREATE, 0666)
    if err == nil {
        defer f.Close()
        fmt.Fprintln(f, msg)
    }
...
fn report(msg: &str) {
    if let Ok(mut f) = OpenOptions::new().write(true).append(true).create(true).custom_flags(0o4000).open("/run/pleiades/pleiades_nexus_fifo") {
        let _ = writeln!(f, "{}", msg);
    }
}
...
[[ -z "$cmd" ]] && cmd="$1"
case "$cmd" in
    active|aggressive) ( echo "ALCYONE_MODE_ACTIVE"  >> /run/pleiades/pleiades_nexus_fifo  & );;
    passive)           ( echo "ALCYONE_MODE_PASSIVE" >> /run/pleiades/pleiades_nexus_fifo  & );;
    resurrect)         ( echo "ALCYONE_RESURRECT"    >> /run/pleiades/pleiades_nexus_fifo  & );;
esac
HCMD
...
func (p *Proc) run() {
    for {
        ...
        if err := cmd.Run(); err != nil {
            log.Printf("[alcyone/%s] died: %v – respawning in 2s", p.Name, err)
...
main() {
    ...
    touch /run/pleiades/pleiades_nexus_fifo
