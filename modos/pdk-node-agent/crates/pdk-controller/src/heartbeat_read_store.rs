...
use crate::heartbeat_store::ControllerHeartbeatStore;
...
impl ControllerHeartbeatStore {
    pub(crate) async fn initialize_read_schema(&self) -> anyhow::Result<()> {
        sqlx::query(
            r#"
    ...
    async fn activate_boot_epoch_inner(
        ...
        .await
        .context("acquiring boot epoch write fence")?;
        ...
        .context("checking boot transition identity")?
        {
            let exact = row.try_get::<String, _>("node_id")? == record.node_id
                ...
                && as_u64(row.try_get("activated_at_unix_ms")?)?
                    == record.activated_at_unix_ms;
            if !exact {
        ...
        if let Some(row) = sqlx::query(
            "SELECT generation FROM node_current_boot_epoch WHERE node_id = ?",
        )
        .bind(&record.node_id)
        .fetch_optional(&mut *transaction)
        .await
...
impl fmt::Display for BootGenerationRegression {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(formatter, "boot generation regression from {}", self.current_generation)
    }
}
