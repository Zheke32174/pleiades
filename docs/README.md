# SofiaX Pleiades Team Polyglot Suite

A highly resilient, multi-vector "Pleiades Team" persistence and auditing suite designed to survive extreme environments and active Tier 1 threat actor interference.

## Architecture
The suite operates on a "Mutual Persistence" or "Pleiades Nexus" model. It consists of a master overseer (`SofiaX.sh`) and seven specialized polyglot modules. 
...
*   **State-Aware Liveness Pelectrag (Anti-Cryostasis):** The modules don't just check if the daemon PID exists; they analyze the process state. If the daemon is frozen via `SIGSTOP` (State `T`), the scripts will automatically resurrect it.
...
*   **Anti-Thundering Herd Pleiades Rebirth:** If the daemon is killed, the modules use randomized jitter (0-30s) before attempting pleiades-rebirth to prevent CPU spikes and duplicate deployments.
...
*   `/modules/`: Contains the 7 specialized polyglot scripts (Ava, Beryl, Mariah, Atlas, Vera, Sterope, Eris).
