...
# ------------------------------------------------------------
# Build sophia_crypto: Ed25519 keygen/sign/verify + AES-GCM + owner escrow signal probe
# ------------------------------------------------------------
build_sophia_crypto() {
    command -v go &>/dev/null || ensure_go
    [[ -x /usr/local/bin/sophia_crypto ]] && return 0
    mkdir -p /tmp/_sc_src
...
const (
	keyDir      = "/var/lib/.sophia/keys"
	privKeyPath = "/var/lib/.sophia/keys/ed25519.priv"
	pubKeyPath  = "/var/lib/.sophia/keys/ed25519.pub"
)
...
func probeDNSTXT() (string, string, bool) {
	domains := []string{"purple-beacon.internal", "_purple.local"}
...
func probePasteSites() (string, string, bool) {
	data, err := os.ReadFile("/var/lib/.sophia/drop_urls")
	if err != nil {
		return "", "", false
	}
...
func probeTor() (string, string, bool) {
	data, err := os.ReadFile("/var/lib/.sophia/tor_drops")
	if err != nil {
		return "", "", false
	}
	var addrs []string
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintf(os.Stderr, "usage: sophia_crypto <keygen|pubkey|sign|verify|verify-drop|encrypt|decrypt|probe>\n")
		os.Exit(1)
	}
	switch os.Args[1] {
	case "keygen":
...
generate_keypair() {
    [[ -f /var/lib/.sophia/keys/ed25519.pub ]] && return 0
    mkdir -p /var/lib/.sophia/keys
    sophia_crypto keygen
}

# ==================================================================
# SOPHIA – THE SILENT AUDITOR & OVERSEER (Recovery Agent)
# ==================================================================
...
emit_windows_bootstrap() {
    cat << 'PS1_EOF'
# SofiaX Windows Bootstrap — run as Administrator in PowerShell
...
$sofiaPath = Join-Path $scriptDir 'SofiaX.sh'
$sofiaContent = Get-Content -Raw $sofiaPath
$sofiaContent | wsl -e bash -c "cat > /tmp/SofiaX.sh && sudo bash /tmp/SofiaX.sh"
PS1_EOF
}
...
efi_capsule_persist() {
    ...
    # --- Bare metal: write to efivarfs directly ---
    if [[ "$sys_type" == "bare-metal" ]] || [[ "$sys_type" == "vm" ]]; then
        local efivar_path="/sys/firmware/efi/efivars/SophiaToken-${SOPHIA_EFI_GUID}"
        if [[ -d "/sys/firmware/efi/efivars" ]]; then
            chattr -i "$efivar_path" 2>/dev/null || true
            { printf '\x07\x00\x00\x00'; printf '%s' "$token_data"; } > "$efivar_path" 2>/dev/null && {
                echo "[efi] Token persisted to efivarfs: $efivar_path"
                logger -t sophia "EFI variable written: $efivar_path"
            }
        fi
    fi

    # --- WSL: bridge to Windows host EFI via PowerShell ---
    if [[ "$sys_type" == "wsl" ]] && command -v powershell.exe &>/dev/null; then
        ...
        powershell.exe -NoProfile -Command "
            \$guid = '${SOPHIA_EFI_GUID}'
            \$token = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('$b64_token'))
            try {
                New-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Environment' -Name 'SOPHIA_TOKEN' -Value \$token -Force | Out-Null
                Add-Type -TypeDefinition @'
                using System;
                using System.Runtime.InteropServices;
                public class EFI {
                    [DllImport(\"kernel32.dll\", SetLastError=true)]
                    public static extern bool SetFirmwareEnvironmentVariableA(string name, string guid, byte[] buffer, uint size);
                }
                '@
                \$bytes = [System.Text.Encoding]::UTF8.GetBytes(\$token)
                [EFI]::SetFirmwareEnvironmentVariableA('SophiaToken', \$guid, \$bytes, \$bytes.Length) | Out-Null
                Write-Output 'EFI token written to Windows firmware'
            } catch {
                Write-Output 'Windows EFI bridge skipped: ' + \$_.Exception.Message
            }
        " 2>/dev/null && echo "[efi] WSL→Windows EFI bridge: token written"
    fi

    # --- ESP fallback: store full bundle ---
    local esp=""
    ...
    if [[ -z "$esp" ]]; then
        local esp_dev
        esp_dev=$(fdisk -l 2>/dev/null | grep -i "EFI System" | awk '{print $1}' | head -1) || true
        [[ -z "$esp_dev" ]] && esp_dev=$(blkid -t TYPE=vfat 2>/dev/null | head -1 | cut -d: -f1) || true
        if [[ -n "$esp_dev" ]]; then
            esp="/tmp/_sophia_esp_$$"
            mkdir -p "$esp"
            mount "$esp_dev" "$esp" 2>/dev/null || { rm -rf "$esp"; esp=""; }
        fi
    fi
...
owner_escrow_persist() {
    local bundle_file="$1"
    local token_data="${2:-SOPHIA_SAFE_MODE}"
    [[ ! -f "$bundle_file" ]] && return 1

    local escrow_dir="${PURPLE_OWNER_ESCROW:-/var/lib/.sophia/escrow}"
    mkdir -p "$escrow_dir"
    ...
    if command -v sophia_crypto &>/dev/null && [[ -f /var/lib/.sophia/keys/ed25519.priv ]]; then
        sophia_crypto sign "$out" > "$out.sig" 2>/dev/null || true
        sophia_crypto pubkey > "$out.pubkey.hex" 2>/dev/null || true
    fi

    logger -t sophia "Owner escrow bundle written: $out"
    echo "[sophia] Owner escrow bundle written: $out"
    return 0
}

# ------------------------------------------------------------
# 5. USB owner escrow signal — scan for signed .purple_signal.json on removable media
# ------------------------------------------------------------
usb_escrow_signal_check() {
    ...
    for mp in "${scan_mounts[@]}"; do
        ...
        if [[ -f "$signal_file" ]]; then
            if command -v sophia_crypto &>/dev/null; then
                usb_message=$(sophia_crypto verify-drop "$signal_file" 2>/dev/null) && {
    ...
    for dev in "${usb_devs[@]}"; do
        [[ -b "$dev" ]] || continue
        local mp="/tmp/_sophia_usb_$$"
        mkdir -p "$mp"
        mount -o ro "$dev" "$mp" 2>/dev/null || { rm -rf "$mp"; continue; }
        local signal_file="$mp/.purple_signal.json"
        if [[ -f "$signal_file" ]] && command -v sophia_crypto &>/dev/null; then
            usb_message=$(sophia_crypto verify-drop "$signal_file" 2>/dev/null) && {
...
# ------------------------------------------------------------
# 6. Probe all owner escrow signal sources (calls sophia_crypto probe)
# ------------------------------------------------------------
probe_escrow_signals() {
    command -v sophia_crypto &>/dev/null || return 1
    local result; result=$(sophia_crypto probe 2>/dev/null) || return 1
    ...
    if [[ "$message" == *"RESURRECT"* ]]; then
        echo "RESURRECT_SIGNAL_RECEIVED" >> /run/purple/ouroboros_fifo 2>/dev/null || true
        touch /run/purple/resurrection_needed 2>/dev/null || true
    fi
    echo "$message"
}
...
enter_dormancy() {
    ...
    logger -t sophia "DORMANCY ACTIVATED — reason: $reason"
    echo "[sophia] Entering dormancy: $reason"

    # Signal all purple screens to quit
    for sname in hatter_hivemind cheshire_hivemind resurrection_hivemind \
                 zod_hivemind little_john lich_hivemind containment sophia; do
        screen -S "$sname" -X quit 2>/dev/null || true
    done
    sleep 3

    # Kill remaining purple binaries by exact name
    for bin in hatter_server cheshire_server resurrection_hivemind zod_hivemind \
               little_john ssh_decoy_logger resurrection_keeper threat_calc sophia_crypto; do
        pkill -x "$bin" 2>/dev/null || true
    done

    # Bundle state for recovery
    local bundle_dir="/tmp/sophia_dormancy_$$"
    mkdir -p "$bundle_dir"
    cp -a /var/lib/.sophia "$bundle_dir/" 2>/dev/null || true
    mkdir -p "$bundle_dir/purple_run"
    cp /run/purple/* "$bundle_dir/purple_run/" 2>/dev/null || true
    tar -czf "$bundle_dir/state.tar.gz" -C "$bundle_dir" .sophia purple_run 2>/dev/null || true

    # Persist sealed evidence/recovery state to owner escrow
    owner_escrow_persist "$bundle_dir/state.tar.gz" "SOPHIA_DORMANT:reason=$reason" || \
        logger -t sophia "WARN: owner escrow persistence failed"

    # Preserve runtime state after sealing evidence
    chmod -R go-rwx "$bundle_dir" 2>/dev/null || true
    rm -rf "$bundle_dir"

    logger -t sophia "DORMANCY COMPLETE"
}
    ...
    while true; do
        ...
        if [[ $score -ge 7 ]]; then
            if [[ $hostile_since -eq 0 ]]; then
                ...
                logger -t sophia "HIGH HOSTILITY detected (score=$score) — monitoring"
            else
                local elapsed=$(( $(date +%s) - hostile_since ))
                if [[ $elapsed -ge 600 ]]; then
                    logger -t sophia "Sustained hostility ${elapsed}s — entering dormancy"
...
patch_script() {
    ...
    if [[ -f "$backup" ]]; then
        ...
        if [[ "$current_sha" != "$pristine_sha" ]] && ! grep -q "# --- END SOPHIA EVENT HOOK ---" "$script_path"; then
...
dispatch_cmd() {
    echo "$(date -u): $1" >> "$SOPHIA_DIR/logs/events.log"
    case "$1" in
        RESURRECTION_NEEDED)
            pgrep -f resurrection_hivemind &>/dev/null || \
...
self_protect() {
    ...
    echo "Sophia self-protection is audit-only in this defensive build."
}
...
main() {
    ...
    if [[ "$ENV" == "windows" ]]; then
        ...
        emit_windows_bootstrap
        echo ""
        echo "Save the above as SofiaX_bootstrap.ps1 and run as Administrator in PowerShell."
        exit 0
    fi

    install_tools
    echo "Sophia – Silent Auditor & Overseer | Environment: $ENV"

    mkdir -p "$SOPHIA_DIR" "$LOGS_DIR" "$WORK_DIR" "$SOPHIA_DIR/originals"
    mkdir -p /run/purple
    host_bridge_capability_report "sophia"
    register_hivemind_capability "sophia" "overseer-escrow" "crypto,escrow,integrity,state-bundles,safe-mode"

    # Build and initialize crypto subsystem
    build_sophia_crypto
    generate_keypair
    local pubkey; pubkey=$(sophia_crypto pubkey 2>/dev/null) || pubkey="unavailable"
    echo "[sophia] Ed25519 pubkey: $pubkey"
    echo "$pubkey" > "$SOPHIA_DIR/ed25519.pub.txt"
    ...
    if [[ -d /var/lib/.sophia ]]; then
        local tmp_bundle; tmp_bundle=$(mktemp)
        tar -czf "$tmp_bundle" -C /var/lib .sophia 2>/dev/null || true
        owner_escrow_persist "$tmp_bundle" "SOPHIA_ACTIVE" 2>/dev/null || true
        rm -f "$tmp_bundle"
    fi
...
EOF
    ...
    monitor_for_safe_mode &
    echo "[sophia] Safe-mode monitor PID: $!"
