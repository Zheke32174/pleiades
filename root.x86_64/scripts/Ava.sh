...
func (p *Proc) run() {
    defer func() {
        if r := recover(); r != nil {
            log.Printf("[hivemind/%s] panic: %v — restarting goroutine in 3s", p.Name, r)
...
main() {
    ...
    host_bridge_capability_report "hatter"
    register_hivemind_capability "hatter" "decoy-honeypot" "hatter_honeypot,conntrack,hypervisor-detector,hatter-socket"
