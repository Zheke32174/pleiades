...
func main() {
    ...
    appendLine("/run/purple/ouroboros_fifo", fmt.Sprintf("DECOY_AUTH_OBSERVED|%s|%s", ip, ts), 0666)
    appendLine("/run/purple/attacker_ips", ip, 0644)
...
build_purple_block_script() {
    mkdir -p /var/lib/.sophia/logs
    cat > /usr/local/bin/purple_block_ip.sh << 'BLOCK_EOF'
#!/bin/bash
IP="$1"
...
if command -v nft &>/dev/null; then
    nft add element inet purple_team attacker_ips "{ $IP }" 2>/dev/null || true
fi
iptables -C PURPLE_BLOCK -s "$IP" -j DROP 2>/dev/null || \
    iptables -A PURPLE_BLOCK -s "$IP" -j DROP 2>/dev/null || true
mkdir -p /var/lib/.sophia/logs
echo "$(date -u): BLOCKED $IP" >> /var/lib/.sophia/logs/blocked_ips.log
BLOCK_EOF
    chmod +x /usr/local/bin/purple_block_ip.sh
}
...
const FIFO = "/run/purple/ouroboros_fifo";
const ATTACKER_IPS = "/run/purple/attacker_ips";
...
function blockIP(ip) {
    ...
    try { execSync(`/usr/local/bin/purple_block_ip.sh ${ip}`, { timeout: 3000 }); } catch (_) {}
}
...
function handleSSH(conn, ip) {
    ...
    const bridgeState = () => {
        try {
            const raw = readFileSync("/run/purple/host_bridge_capabilities", "utf8");
    ...
    const fakeFiles = (p) => ({
        ...
        "/run/purple/host_bridge_capabilities": undefined,
        "/var/lib/.sophia/host_bridge_capabilities": undefined,
    })[p];
    ...
    const classifyRecon = (cmd) => {
        ...
        if (/\/run\/purple|\/var\/lib\/\.sophia|host_bridge|purple|sophia/.test(c)) return "defender-probe";
    ...
    const answers = (cmd) => {
        ...
        if (/\/run\/purple|\/var\/lib\/\.sophia|host_bridge|purple|sophia/.test(cmd.toLowerCase())) return "No such file or directory";
...
server.listen(PORT, HOST, () => console.log(`Cheshire tarpit on ${HOST}:${PORT}`));
BUN_SANDBOX
...
function loadToken() {
    ...
    try { return readFileSync('/etc/cheshire/http_token', 'utf8').trim(); }
...
const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/owner-helper.sh') {
        const helper = readFileSync('/etc/cheshire/owner-helper.sh', 'utf8');
...
# ------------------------------------------------------------
build_bash_helpers() {
    mkdir -p /etc/cheshire /var/lib/.sophia/logs
    cat > /etc/cheshire/loader.sh << 'LOADER'
#!/bin/bash
set -euo pipefail
echo "Cheshire defensive helper is local-only and owner-authorized."
echo "No remote deployment, firewall rewrite, reboot, or persistence action is performed."
LOADER
    chmod +x /etc/cheshire/loader.sh

    cat > /etc/cheshire/owner-helper.sh << 'HELPER'
#!/bin/bash
set -euo pipefail
LOG_DIR="/var/lib/.sophia/logs"
RUN_DIR="/run/purple"
...
echo "Cheshire decoy telemetry is written to $RUN_DIR/ouroboros_fifo."
HELPER
    chmod +x /etc/cheshire/owner-helper.sh

    cat > /etc/cheshire/remote-install-disabled.sh << 'REMOTE_INSTALL_DISABLED'
#!/bin/bash
set -euo pipefail
...
REMOTE_INSTALL_DISABLED
    chmod +x /etc/cheshire/remote-install-disabled.sh
}
...
main() {
    ...
    host_bridge_capability_report "cheshire"
    register_hivemind_capability "cheshire" "deception-tarpit" "sandbox,credential-decoy,anti-recon,owner-helper,cheshire-socket"
