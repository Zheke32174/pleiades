...
# OUROBOROS_ID
# ==================================================================
# OUROBOROS containment – OMNIVERSAL (WSL / DGX Spark / VPS)
# ==================================================================
...
func contain() {
    ...
    contained = true
    log.Println("Threat threshold exceeded – initiating Ouroboros containment")
    reportToOuroboros("CONTAINMENT_TRIGGERED")
...
#!/bin/bash
set -euo pipefail
STATE_FILE="${PURPLE_HOST_BRIDGE_STATE:-/run/purple/host_bridge_capabilities}"
FIFO="/run/purple/ouroboros_fifo"
BASE_DIR="/var/lib/purple-team/host-bridge"
mkdir -p "$BASE_DIR" /run/purple
...
After=network.target ouroboros-omniversal.service
...
set -euo pipefail
FIFO="/run/purple/ouroboros_fifo"
BASE="/var/lib/purple-team/host-bridge/windows11"
PS="/host/mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe"
mkdir -p "$BASE" /run/purple
touch "$FIFO"
...
# ------------------------------------------------------------
# 8. Hivemind substrate control plane (deterministic, policy-gated)
# ------------------------------------------------------------
build_hivemind_substrate() {
    mkdir -p /etc/purple /run/purple/{requests,decisions,actions,results,capabilities,state,alien/inbox,alien/outbox} /var/lib/purple-team/hivemind
    ...
    cat > /etc/purple/host-bridge-policy.json <<'HOST_POLICY'
{
...
HOST_POLICY
    ...
    if [[ ! -f /etc/purple/hivemind-policy.json ]]; then
        cat > /etc/purple/hivemind-policy.json <<'POLICY'
{
  "schema": "purple-hivemind-policy-v1",
  "mode": "owner-authorized-defensive",
  "default_request_decision": "deny",
...
POLICY
    ...
    cat > /usr/local/bin/purplectl <<'PURPLECTL'
#!/bin/bash
set -euo pipefail
RUN="/run/purple"
POLICY="/etc/purple/hivemind-policy.json"
usage() {
    cat <<'USAGE'
...
event() { printf '%s\n' "$1" >> "$RUN/ouroboros_fifo" 2>/dev/null || true; }
...
status() {
    ...
    if [[ -f /host/run/purple-gentoo-heartbeat/status ]]; then
        sed 's/^/deployment_heartbeat./' /host/run/purple-gentoo-heartbeat/status
    fi
}
health() {
    status
    for svc in cheshire-omniversal hatter-omniversal resurrection-omniversal zod-omniversal little-john-omniversal robin-omniversal ouroboros-omniversal sophia host-bridge-monitor windows-host-bridge-monitor purple-adaptive-builder purple-request-broker; do
...
schema=purple-hivemind-request-v1
id=$id
origin=purplectl
class=$class
scope=$scope
...
REQ
    event "HIVEMIND_REQUEST|$id|$class|$scope|$action"
    echo "$id"
}
list_or_cat() {
    local dir="$1" id="${2:-}"
...
alien_status() {
    echo "schema=purple-alien-dock-v1"
    echo "enabled=false"
...
host_process_summary() {
    ...
    echo "updated_utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo "policy=/etc/purple/host-bridge-policy.json"
    if [[ -f /host/run/purple-host-capsule/status ]]; then
        echo "--- capsule_status ---"
        cat /host/run/purple-host-capsule/status
    else
        echo "capsule_status=absent"
    fi
    if [[ -f /host/run/purple-host-capsule/process-summary ]]; then
        echo "--- process_summary ---"
        cat /host/run/purple-host-capsule/process-summary
    else
...
case "${1:-}" in
    ...
    events) tail -n "${2:-80}" "$RUN/ouroboros_fifo" 2>/dev/null || true ;;
...
PURPLECTL
    chmod +x /usr/local/bin/purplectl

    cat > /usr/local/bin/purple_request_broker.sh <<'BROKER'
#!/bin/bash
set -euo pipefail
RUN="/run/purple"
POLICY="/etc/purple/hivemind-policy.json"
mkdir -p "$RUN/requests" "$RUN/decisions" "$RUN/actions" "$RUN/results" "$RUN/alien/inbox" "$RUN/alien/outbox"
touch "$RUN/ouroboros_fifo"
event() { printf '%s\n' "$1" >> "$RUN/ouroboros_fifo" 2>/dev/null || true; }
kv_get() { grep -E "^$2=" "$1" 2>/dev/null | tail -1 | cut -d= -f2-; }
allowed_class() {
...
schema=purple-hivemind-decision-v1
id=$id
decision=$decision
reason=$reason
policy=$POLICY
...
schema=purple-hivemind-result-v1
id=$id
status=$status
detail=$detail
updated_utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)
...
process_request() {
    ...
    case "$class" in
        status|health) /usr/local/bin/purplectl health > "$RUN/results/$id.output" 2>&1 || true ;;
        capabilities) /usr/local/bin/purplectl capabilities > "$RUN/results/$id.output" 2>&1 || true ;;
        evidence-list) find /var/lib/.sophia/escrow /var/lib/purple-team -maxdepth 3 -type f 2>/dev/null | sort > "$RUN/results/$id.output" || true ;;
        brl-status) /usr/local/bin/purplectl brl-status > "$RUN/results/$id.output" 2>&1 || true ;;
        strat-list) /usr/local/bin/purplectl strat-list > "$RUN/results/$id.output" 2>&1 || true ;;
        host-process-summary) /usr/local/bin/purplectl host-process-summary > "$RUN/results/$id.output" 2>&1 || true ;;
...
event "HIVEMIND_BROKER_READY|policy-gated|alien-advisory-only"
while true; do
...
BROKER
    chmod +x /usr/local/bin/purple_request_broker.sh

    cat > /run/purple/capabilities/gentoo_container.cap <<'CAP'
schema=purple-hivemind-capability-v1
...
source=/host/run/purple-gentoo-heartbeat/status
authority=deployment-layer
ai_sidecar_required=no
CAP
    cat > /run/purple/capabilities/host_capsule.cap <<'CAP'
schema=purple-hivemind-capability-v1
...
source=/host/run/purple-host-capsule/status
policy=/etc/purple/host-bridge-policy.json
authority=observe-and-report
ai_sidecar_required=no
CAP
    cat > /run/purple/capabilities/brl_strat.cap <<'CAP'
schema=purple-hivemind-capability-v1
component=brl_strat
...
ai_sidecar_required=no
CAP
    cat > /run/purple/capabilities/alien_placeholder.cap <<'CAP'
schema=purple-hivemind-capability-v1
...
ai_sidecar_required=no
CAP
    cat > /run/purple/capabilities/quantum_placeholder.cap <<'CAP'
schema=purple-hivemind-capability-v1
component=quantum_placeholder
...
ai_sidecar_required=no
CAP
    printf 'HIVEMIND_SUBSTRATE_READY|purplectl|broker|brl-strat|gentoo-container|quantum-placeholder|alien-placeholder\n' >> /run/purple/ouroboros_fifo 2>/dev/null || true
}

install_hivemind_substrate() {
    if ! systemd_usable; then
        pkg_install screen
        screen -dmS purple_request_broker /usr/local/bin/purple_request_broker.sh
    else
        cat > /etc/systemd/system/purple-request-broker.service << SERVICE
[Unit]
Description=Purple Hivemind Policy-Gated Request Broker
After=network.target ouroboros-omniversal.service

[Service]
Type=simple
ExecStart=/usr/local/bin/purple_request_broker.sh
Restart=always
RestartSec=5
User=root
...
SERVICE
        systemctl daemon-reload
        systemctl enable purple-request-broker.service
        systemctl restart purple-request-broker.service
    fi
}
...
build_adaptive_tool_builder() {
    cat > /usr/local/bin/purple_adaptive_builder.sh << 'ADAPTIVE_BUILDER'
#!/bin/bash
set -euo pipefail

FIFO="/run/purple/ouroboros_fifo"
STATE_DIR="/var/lib/purple-team/adaptive"
TOOLS_DIR="/usr/local/lib/purple-tools"
BIN_DIR="/usr/local/bin"
mkdir -p "$STATE_DIR" "$TOOLS_DIR" /run/purple
touch "$FIFO"
...
build_recipe() {
    ...
    case "$category" in
        network_scan)
            write_tool purple-net-summary '#!/bin/bash
set -euo pipefail
mkdir -p /var/lib/purple-team/adaptive/reports
out="/var/lib/purple-team/adaptive/reports/net_$(date -u +%Y%m%dT%H%M%SZ).txt"
...
echo "ADAPTIVE_REPORT|network_scan|$out" >> /run/purple/ouroboros_fifo 2>/dev/null || true
printf "%s\n" "$out"'
            ;;
        web_probe)
            write_tool purple-web-triage '#!/bin/bash
set -euo pipefail
mkdir -p /var/lib/purple-team/adaptive/reports
out="/var/lib/purple-team/adaptive/reports/web_$(date -u +%Y%m%dT%H%M%SZ).txt"
{ date -u; ss -tanp 2>/dev/null | awk "/:80|:443|:8080|:8443/" || true; openssl version 2>/dev/null || true; } > "$out"
echo "ADAPTIVE_REPORT|web_probe|$out" >> /run/purple/ouroboros_fifo 2>/dev/null || true
printf "%s\n" "$out"'
            ;;
        auth_abuse)
            write_tool purple-auth-triage '#!/bin/bash
set -euo pipefail
mkdir -p /var/lib/purple-team/adaptive/reports
out="/var/lib/purple-team/adaptive/reports/auth_$(date -u +%Y%m%dT%H%M%SZ).txt"
...
echo "ADAPTIVE_REPORT|auth_abuse|$out" >> /run/purple/ouroboros_fifo 2>/dev/null || true
printf "%s\n" "$out"'
            ;;
        dns_route_anomaly)
            write_tool purple-route-triage '#!/bin/bash
set -euo pipefail
mkdir -p /var/lib/purple-team/adaptive/reports
out="/var/lib/purple-team/adaptive/reports/route_$(date -u +%Y%m%dT%H%M%SZ).txt"
...
echo "ADAPTIVE_REPORT|dns_route_anomaly|$out" >> /run/purple/ouroboros_fifo 2>/dev/null || true
printf "%s\n" "$out"'
            ;;
        host_bridge_probe|container_probe)
            write_tool purple-host-bridge-triage '#!/bin/bash
set -euo pipefail
mkdir -p /var/lib/purple-team/adaptive/reports
out="/var/lib/purple-team/adaptive/reports/host_bridge_$(date -u +%Y%m%dT%H%M%SZ).txt"
{ date -u; cat /run/purple/host_bridge_capabilities 2>/dev/null || true; mount 2>/dev/null | grep -E "host|mnt/c|docker|systemd" || true; systemd-detect-virt --container 2>/dev/null || true; } > "$out"
echo "ADAPTIVE_REPORT|host_bridge_probe|$out" >> /run/purple/ouroboros_fifo 2>/dev/null || true
printf "%s\n" "$out"'
            ;;
        resource_pressure)
            write_tool purple-resource-triage '#!/bin/bash
set -euo pipefail
mkdir -p /var/lib/purple-team/adaptive/reports
out="/var/lib/purple-team/adaptive/reports/resource_$(date -u +%Y%m%dT%H%M%SZ).txt"
{ date -u; uptime; ps aux --sort=-%cpu 2>/dev/null | head -25; ps aux --sort=-%mem 2>/dev/null | head -25; vmstat 1 3 2>/dev/null || true; } > "$out"
echo "ADAPTIVE_REPORT|resource_pressure|$out" >> /run/purple/ouroboros_fifo 2>/dev/null || true
printf "%s\n" "$out"'
            ;;
        evidence_pack)
            write_tool purple-evidence-pack '#!/bin/bash
set -euo pipefail
mkdir -p /var/lib/purple-team/adaptive/bundles
out="/var/lib/purple-team/adaptive/bundles/evidence_$(date -u +%Y%m%dT%H%M%SZ).tar.gz"
tar -czf "$out" /run/purple /var/lib/purple-team/adaptive/reports 2>/dev/null || true
echo "ADAPTIVE_REPORT|evidence_pack|$out" >> /run/purple/ouroboros_fifo 2>/dev/null || true
printf "%s\n" "$out"'
            ;;
    esac
...
ADAPTIVE_BUILDER
    chmod +x /usr/local/bin/purple_adaptive_builder.sh
}

install_adaptive_tool_builder() {
    if ! systemd_usable; then
        pkg_install screen
        screen -dmS purple_adaptive_builder /usr/local/bin/purple_adaptive_builder.sh
    else
        cat > /etc/systemd/system/purple-adaptive-builder.service << SERVICE
[Unit]
Description=Purple Adaptive Defensive Tool Builder
After=network.target ouroboros-omniversal.service host-bridge-monitor.service

[Service]
Type=simple
ExecStart=/usr/local/bin/purple_adaptive_builder.sh
Restart=always
RestartSec=10
User=root
...
SERVICE
        systemctl daemon-reload
        systemctl enable purple-adaptive-builder.service
        systemctl start purple-adaptive-builder.service
    fi
}
...
install_service() {
    if ! systemd_usable; then
        pkg_install screen
        screen -dmS ouroboros /usr/local/bin/containment_controller
    else
        cat > /etc/systemd/system/ouroboros-omniversal.service << SERVICE
[Unit]
Description=Ouroboros containment Omniversal
...
main() {
    ...
    mkdir -p /run/purple
    host_bridge_capability_report "ouroboros"
    register_hivemind_capability "ouroboros" "containment-substrate" "containment-controller,host-bridge-monitor,windows-bridge-monitor,adaptive-builder,hivemind-substrate,brl-strat-registry,alien-dock-placeholder"
