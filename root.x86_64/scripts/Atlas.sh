...
#!/bin/bash
set -euo pipefail
# Source configuration for path overrides
CONFIG="/etc/purple/pleiades.conf"
[[ -f "$CONFIG" ]] && . "$CONFIG"
FIFO="/run/pleiades/pleiades-nexus_fifo"
BASE="/var/lib/pleiades-team/host-bridge/windows11"
PS="${PLEIADES_POWERSHELL:-/host/mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe}"
...
build_pleiades-swarm_substrate() {
    mkdir -p /etc/pleiades ${PLEIADES_RUN_DIR}/{requests,decisions,actions,results,capabilities,state,alien/inbox,alien/outbox} /var/lib/pleiades-team/pleiades-swarm
...
HOST_POLICY
    ...
    if [[ ! -f ${PLEIADES_POLICY_DIR}/pleiades-swarm-policy.json ]]; then
...
status() {
    ...
    echo "components=alcyone,taygete,electra,pleiades-rebirth,atlas,celaeno,pleiades-nexus,maia,host-bridge-monitor,windows-host-bridge-monitor,pleiades-adaptive-builder,pleiades-request-broker"
    echo "policy=$POLICY"
...
write_host_bridge_state() {
    ...
    [[ -e $PLEIADES_POWERSHELL ]] && windows_host_files="$PLEIADES_WINDOWS_ROOT"
...
monitor_threats() {
    while true; do
        if bgp_hijack_detected; then
            log_json "EVENT" "pleiades-nexus" "BGP hijack detected – setting flag"
            touch ${PLEIADES_RUN_DIR}/bgp_hijack
        fi
        if thermal_anomaly; then
            log_json "EVENT" "pleiades-nexus" "Thermal anomaly detected – setting flag"
        ...
        for tool in volatility volatility3 rekall strings gdb; do
            if command -v "$tool" &>/dev/null; then
                log_json "EVENT" "pleiades-nexus" "Forensic tool detected: $tool"
                ( echo "ANOMALY|forensic_tool|$tool" >> ${PLEIADES_RUN_DIR}/pleiades-nexus_fifo & )
fi
        done
        sleep 30
    done
