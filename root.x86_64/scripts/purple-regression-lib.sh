#!/usr/bin/env bash
# Advanced test library for purple-regression.sh
...
test_recon_replay() {
    ...
    before="$(in_container "wc -l < /run/purple/ouroboros_fifo 2>/dev/null || echo 0")" || before=0

    # Probe Cheshire (port 2222) with identity recon commands
    ...
    after="$(in_container "wc -l < /run/purple/ouroboros_fifo 2>/dev/null || echo 0")" || after=0

    if [[ "$after" -gt "$before" ]]; then
        pass "HOSTILE_RECON events emitted to ouroboros_fifo (lines: $before → $after)"
    else
        fail "no new telemetry lines in ouroboros_fifo after recon probes"
    fi
}
...
test_broker_deny_matrix() {
    ...
    local req_file="/run/purple/requests/${req_id}.req"
    local decision_dir="/run/purple/decisions"
    ...
    in_container "rm -f '${req_file}' '${decision_dir}/${req_id}'* '/run/purple/results/${req_id}'* 2>/dev/null" || true

    # Test 2: allowed class (capabilities)
    local req_id2="test-allow-$$"
    local req_file2="/run/purple/requests/${req_id2}.req"
    ...
    in_container "rm -f '${req_file2}' '${decision_dir}/${req_id2}'* '/run/purple/results/${req_id2}'* 2>/dev/null" || true
}
...
test_host_bridge() {
    ...
    local snap_dir="/var/lib/purple-team/host-bridge/windows11"
    local recent
...
# ── subtask 5b: Little John liveness ─────────────────────────────────────────

test_little_john_alive() {
    if ! container_up; then skip "Little John (container down)"; return; fi

    # Service active
    if in_container "systemctl is-active little-john-omniversal.service" &>/dev/null; then
        pass "little-john-omniversal.service is active"
    else
        fail "little-john-omniversal.service is not active"
        return
    fi

    # Command file writable (Little John reads /run/purple/little_john_cmd)
    if in_container "test -w /run/purple/little_john_cmd 2>/dev/null || test -e /run/purple/little_john_cmd"; then
        pass "/run/purple/little_john_cmd exists"
    else
        fail "/run/purple/little_john_cmd missing"
    fi
    ...
    local restarts
    restarts="$(in_container "journalctl -u little-john-omniversal.service --since '2 minutes ago' --no-pager 2>/dev/null | { grep -c 'Started\|start request' || true; }")" || restarts=0
    restarts="${restarts//[^0-9]/}"   # strip any stray whitespace/newlines
    restarts="${restarts:-0}"
    if [[ "$restarts" -le 2 ]]; then
        pass "Little John restart count in last 2m: ${restarts} (≤2)"
    else
        fail "Little John crash-looping: ${restarts} restarts in last 2m"
    fi
}

# ── subtask 5c: Sophia crypto round-trip ─────────────────────────────────────

test_sophia_crypto() {
    if ! container_up; then skip "Sophia crypto (container down)"; return; fi

    if ! in_container "test -x /usr/local/bin/sophia_crypto" &>/dev/null; then
        fail "sophia_crypto binary not found or not executable"
        return
    fi

    # sophia_crypto sign <file>  → hex signature
    # sophia_crypto verify <file> <sigHex>  → exit 0 on valid
    local tmp_msg="/tmp/_purple_regtest_msg_$$"
    local test_payload="regression-test-$(date +%s)"
    in_container "printf '%s' '${test_payload}' > '${tmp_msg}'" 2>/dev/null || true

    local signed
    signed="$(in_container "/usr/local/bin/sophia_crypto sign '${tmp_msg}' 2>/dev/null")" || true

    if [[ -z "$signed" ]]; then
        in_container "rm -f '${tmp_msg}'" 2>/dev/null || true
        fail "sophia_crypto sign failed (empty output)"
        return
    fi
    pass "sophia_crypto sign succeeded"

    # Verify the signature: pass file + sigHex as separate args
    local verify_rc
    verify_rc="$(in_container "/usr/local/bin/sophia_crypto verify '${tmp_msg}' '${signed}' 2>/dev/null; echo \$?")" || verify_rc=1
    verify_rc="$(echo "$verify_rc" | tail -1 | tr -d '[:space:]')"

    in_container "rm -f '${tmp_msg}'" 2>/dev/null || true

    if [[ "$verify_rc" == "0" ]]; then
        pass "sophia_crypto verify round-trip: OK"
    else
        fail "sophia_crypto verify round-trip: FAILED (rc=${verify_rc})"
    fi
}
