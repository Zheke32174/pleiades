#!/bin/bash
# purple-selfdestruct.sh — Evidence-preserving self-destruct + auto-redeploy trigger
#
# Sequence:
...
#
# Usage:
#   purple-selfdestruct.sh [--wipe-only] [--no-push] [--redeploy] [--dry-run]
...
EVIDENCE_SOURCES=(
    "/var/lib/.sophia/logs"
    "/var/log/purple-*.log"
    "/var/log/purple-regression*"
    "/run/purple-host-capsule/process-alerts.jsonl"
    "/var/lib/.sophia/state.json"
...
encrypt_bundle() {
    ...
    if ! $DRY_RUN; then
        # Derive key from machine identity (same derivation as SofiaX.sh)
