...
# Usage: bash purple-regression.sh [--skip-container]
# Sources: purple-regression-lib.sh (advanced tests)

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPORT_DIR="/var/log/purple-regression"
PASS=0; FAIL=0; SKIP=0
SKIP_CONTAINER="${1:-}"

SCRIPTS=(SofiaX Beryl Ava Eris Mariah Artemis Vera Zara)
CONTAINER_PID=""
...
test_syntax_and_units() {
    ...
    units="$(in_container "systemctl list-unit-files --state=enabled,static \
        | awk '{print \$1}' \
        | grep -E '(-omniversal|purple-|sophia|host-bridge-monitor|windows-host-bridge-monitor)\.service' \
        | tr '\n' ' '")" || true
...
# ── subtask 3: decoy port liveness + Cheshire concurrency cap ────────────────

test_ports_and_concurrency() {
    if ! container_up || [[ "$SKIP_CONTAINER" == "--skip-container" ]]; then
        skip "port liveness (container down)"
        skip "Cheshire concurrency cap (container down)"
        return
    fi
    ...
    echo "-- Cheshire concurrency cap (MAX_CONNS_PER_IP=8) --"
    ...
    if [[ "$banner_count" -le 8 ]]; then
        pass "Cheshire concurrency cap: ${banner_count}/12 got banner (≤8 expected)"
    else
        fail "Cheshire concurrency cap: ${banner_count}/12 got banner (>8 — cap not enforced)"
    fi
}
...
# --- ADVANCED TESTS SOURCED FROM purple-regression-lib.sh ---
...
LIB="$SCRIPT_DIR/purple-regression-lib.sh"
if [[ -f "$LIB" ]]; then
    # shellcheck source=purple-regression-lib.sh
    source "$LIB"
    ...
    run_group "Little John liveness"         test_little_john_alive
    run_group "Sophia crypto round-trip"     test_sophia_crypto
    # Concurrency cap runs last — floods 127.0.0.1 and may trigger AGGRESSIVE mode
    run_group "Decoy port liveness + Cheshire concurrency" test_ports_and_concurrency
else
    run_group "Decoy port liveness + Cheshire concurrency" test_ports_and_concurrency
    skip "Advanced tests (purple-regression-lib.sh not found)"
fi
