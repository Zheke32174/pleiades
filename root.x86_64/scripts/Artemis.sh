...
func parseLine(line string) {
    ...
    } else if strings.Contains(line, "RESURRECTION_TRIGGERED") {
        resurrectionNeeded = true
        reportToOuroboros("ZOD_RESURRECTION_ACK")
        os.WriteFile("/run/purple/resurrection_acknowledged", []byte("done"), 0644)
    } else if strings.Contains(line, "CONTAINMENT_TRIGGERED") {
        containmentTriggered = true
        atomic.StoreInt64(&threatScore, 0)
        reportToOuroboros("ZOD_containment_ACK")
    }
}
...
func main() {
    ...
    for {
        ...
        if score >= 8 {
            modeStr = "AGGRESSIVE"
            sendCommand("/run/purple/cheshire.sock", "aggressive")
            sendCommand("/run/purple/hatter.sock", "active")
            reportToOuroboros(fmt.Sprintf("ZOD_MODE_AGGRESSIVE|%d", score))
        } else if score >= 2 {
            modeStr = "PASSIVE"
            sendCommand("/run/purple/hatter.sock", "passive")
            reportToOuroboros(fmt.Sprintf("ZOD_MODE_PASSIVE|%d", score))
        } else {
            modeStr = "NORMAL"
        }
        os.WriteFile("/run/purple/zod_mode", []byte(modeStr), 0644)
        if resurrectionNeeded && !containmentTriggered {
            sendCommand("/run/purple/cheshire.sock", "resurrect")
            sendCommand("/run/purple/hatter.sock", "resurrect")
            reportToOuroboros("ZOD_RESURRECTION_SIGNAL")
...
func reportToOuroboros(msg string) {
    f, _ := os.OpenFile("/run/purple/ouroboros_fifo", os.O_WRONLY|os.O_APPEND|syscall.O_NONBLOCK, 0666)
    if f != nil {
        defer f.Close()
        fmt.Fprintln(f, msg)
    }
...
`, ip, thrallMaxFloods, thrallInterval, banner)
    ...
    cmd.Start()
    reportToOuroboros(fmt.Sprintf("THRALL_DEPLOYED|%s", ip))
}
...
func main() {
    ...
    var offset int64
    for {
        f, err := os.Open("/run/purple/attacker_ips")
        if err == nil {
            fi, _ := f.Stat()
            if fi.Size() > offset {
                f.Seek(offset, io.SeekStart)
...
#!/bin/bash
# Tails ouroboros log and sends commands to Hatter/Cheshire sockets
unix_send() { printf '%s\n' "$1" | socat - "UNIX-CONNECT:$2" 2>/dev/null || printf '%s\n' "$1" | nc -U "$2" -w 1 2>/dev/null || true; }
tail -n 0 -F /run/purple/ouroboros_fifo 2>/dev/null | while read -r line; do
    if [[ "$line" =~ ^ZOD_MODE_(AGGRESSIVE|PASSIVE) ]]; then
        mode=$(echo "$line" | cut -d'|' -f1 | cut -d'_' -f3)
        if [[ "$mode" == "AGGRESSIVE" ]]; then
            unix_send "aggressive" /run/purple/cheshire.sock
            unix_send "active" /run/purple/hatter.sock
        elif [[ "$mode" == "PASSIVE" ]]; then
            unix_send "passive" /run/purple/hatter.sock
        fi
    fi
done
SWITCH
...
# ------------------------------------------------------------
# 7. Install systemd service (Zod hivemind)
# ------------------------------------------------------------
install_systemd() {
    if ! systemd_usable; then
        pkg_install screen
        screen -dmS zod_hivemind /usr/local/bin/zod_hivemind
    else
        cat > /etc/systemd/system/zod-omniversal.service << SERVICE
[Unit]
Description=General Zod Omniversal Orchestrator
After=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/zod_hivemind
Restart=always
RestartSec=5
...
SERVICE
    ...
    # Build Go hivemind that runs threat calculator and thrall deployer
    cat > /tmp/zod_hivemind.go << 'GO_HIVE'
package main
...
GO_HIVE
    go build -o /usr/local/bin/zod_hivemind /tmp/zod_hivemind.go
    chmod +x /usr/local/bin/zod_hivemind
    rm -f /tmp/zod_hivemind.go
}
...
monitor_threats() {
    while true; do
        if bgp_hijack_detected; then
            logger -t zod "BGP hijack detected – signalling Ouroboros"
            ( echo "BGP_HIJACK" >> /run/purple/ouroboros_fifo & )
fi
        if thermal_anomaly; then
            logger -t zod "Thermal anomaly detected – signalling Ouroboros"
...
main() {
    ...
    mkdir -p /etc/zod /run/purple
    host_bridge_capability_report "zod"
    register_hivemind_capability "zod" "threat-orchestrator" "threat-scoring,mode-switch,thrall-dispatch"
    wait_for hatter 2
    wait_for cheshire 2
    build_go_threat_calc
    build_go_thrall
...
INST
    ...
    signal_ready zod
    echo "General Zod Omniversal deployed on $ENV."
}
