...
STATE_DIR="${PLEIADES_HOST_CAPSULE_STATE:-/var/lib/pleiades-host-capsule}"
...
event() {
    ...
    jq -c -n --arg ts "$ts" --arg cat "$category" --arg pid "$pid" \
          ...
          --arg cmd "$cmd" \
          '{schema: "pleiades-host-process-alert-v1", updated_utc: $ts, category: $cat, pid: ($pid|tonumber), name: $name, score: ($score|tonumber), reason: $reason, cmdline: $cmd}' \
          >> "$ALERTS"
}
...
collect() {
    ...
    {
        echo "schema=pleiades-host-process-summary-v1"
